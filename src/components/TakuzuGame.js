...
const TakuzuGame = () => {
  ...
  const checkAllRowsAndColumns = (board) => {
    for (let i = 0; i < board.length; i++) {
      const row = board[i].map(cell => cell.value);
      const column = board.map(row => row[i].value);
      if (!checkNoThreeConsecutive(row) || !checkEqualNumberOfSymbols(row) ||
          !checkNoThreeConsecutive(column) || !checkEqualNumberOfSymbols(column)) {
        return false;
      }
    }
    return true;
  ...
  const checkWinner = (currentBoard) => {
    ...
    if (isBoardFull && checkAllRowsAndColumns(currentBoard) && checkUniqueRowsAndColumns(currentBoard)) {
      return true;
    }
    return false;
  };
          ...
          <Row key={rowIndex}>
            {row.map((cell, colIndex) => (
              <Col key={colIndex} onClick={() => handleCellClick(rowIndex, colIndex)}>
                <div className={`
                  cell 
                  ${cell.value === null ? 'empty' : cell.value === 0 ? (cell.isAutoGenerated ? 'cell-auto-zero' : 'cell-manual-zero') : (cell.isAutoGenerated ? 'cell-auto-one' : 'cell-manual-one')}
                `}>
